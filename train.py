        ...
        for ep in range(args.epochs):
            ...
            for i in range(0, len(data), args.batch_size):
                ...
                for obj_name in OBJ_NAMES:
                    ...
                    all_grads[obj_name] = (
                        torch.cat(flat_all, dim=0).to(device) if len(flat_all) > 0 else torch.zeros(1, device=device)
                    )
                ...
                g_keys, G = cosine_matrix(all_grads) if len(all_grads) > 0 else ([], torch.zeros(0, 0, device=device))
